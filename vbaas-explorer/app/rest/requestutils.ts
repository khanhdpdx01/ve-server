...
export const queryDatevalidator = function(from: string, to: string) {
	if (!isNaN(Date.parse(from)) && !isNaN(Date.parse(to))) {
		...
		from = createDateAsUTC(new Date(new Date().getMilliseconds() - 864e5)).toISOString();
		to = createDateAsUTC(new Date()).toISOString();
	}
	return { from, to };
};


const createDateAsUTC = function (date: Date) {
